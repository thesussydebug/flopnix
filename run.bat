...
exit /b %errorlevel%
:withusb
"%QEMU%" -drive if=floppy,format=raw,file="%IMAGE%" -boot a -m 12 -rtc base=localtime -netdev %NET% -device ne2k_pci,netdev=n0 -device piix3-usb-uhci,id=uhci -drive if=none,id=stick,format=raw,file="%USB%" -device usb-storage,bus=uhci.0,drive=stick
exit /b %errorlevel%
