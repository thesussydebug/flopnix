...
for sym in $(undefs out/emergency.o); do
    case "$sym" in thr_self|emergency_enter|kupd_critical|timer_alive) ;;
...
cat > built/README.txt <<TXT
FLOPNIX build artifacts (regenerated by build.sh)

flopnix.img   whole-disk 1.44 MB image - write to a floppy for a fresh install
